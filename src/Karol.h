...
BodyPart BodyPart_New(vec3d a,vec3d p,short sides,vec3d start,vec3d l,unsigned int col){
    ...
    bp.p = p;
    bp.a = a;
    bp.mesh = Vector_New(sizeof(triangle));
...
void Karol_Add(Karol* k,Vector* tris){
    mat4x4 matY = Matrix_MakeRotationY(k->a.y);
    mat4x4 matX = Matrix_MakeRotationX(k->a.x);
    
	for(int i = 0;i<k->bodyparts.size;i++){
        BodyPart* bp = (BodyPart*)Vector_Get(&k->bodyparts,i);
        
        for(int j = 0;j<bp->mesh.size;j++) {
            triangle* tri = (triangle*)Vector_Get(&bp->mesh,j);

			mat4x4 matTurnX = Matrix_MakeRotationX(bp->a.x);
			mat4x4 matTurnY = Matrix_MakeRotationY(bp->a.y);
			mat4x4 matTurnZ = Matrix_MakeRotationZ(bp->a.z);
            ...
            triangle_CalcNorm(&triRotated);
            //triangle_ShadeNorm(&triRotated,(vec3d){ 0.6f,0.7f,0.8f });
